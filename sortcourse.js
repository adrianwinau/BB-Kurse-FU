...
function main() {  
  ...
  let courseList = courseDiv.querySelectorAll("#" + courseId + " > ul > li");//coursesList
  ...
  for( i = 0; i < courseList.length; i++){
    ...
    courseList[i].innerHTML = courseList[i].innerHTML.replace(id + ": ",'')
    ...
    let x = [
      ...
      courseText.substring(courseText.indexOf(':')+2, courseText.indexOf('\n')) // Name
    ];
    if( !semsterpattern.test(x[1]) ){ x[1] = '00A'; }
    courses.push(x);
  }
  ...
  while( !courses.isEmpty() ){
    ...
    if( !(semester === x[1])  ){
      ...
      h2.innerHTML = printSemster(semester);
      newCoursesDiv.innerHTML += h2.outerHTML;
    }
    newCoursesDiv.innerHTML += x[0].outerHTML;
  }
